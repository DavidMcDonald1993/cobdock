...
from cob_dock.binding_site_identification.fpocket.fpocket_utils import run_fpocket_and_collate_single_target
from cob_dock.binding_site_identification.p2rank.p2rank_utils import run_p2rank_and_collate_single_target

from cob_dock.docking.vina.vina_utils import VINA_N_PROC, prepare_ligand_for_vina, prepare_target_for_vina, VINA_VARIANTS
from cob_dock.docking.plants.plants_utils import PLANTS_N_PROC, prepare_for_plants
from cob_dock.docking.galaxydock.galaxydock_utils import GALAXYDOCK_N_PROC, prepare_ligand_for_galaxydock, prepare_target_for_galaxydock
...
def get_number_of_entities_in_pdb_file(
    pdb_ids,
    mapping_csv_filename: str = "http://ftp.ebi.ac.uk/pub/databases/msd/sifts/csv/pdb_chain_uniprot.csv", # source is ftp.ebi.ac.uk/pub/databases/msd/sifts/csv/pdb_chain_uniprot.csv
    verbose: bool = False,
    ):
    if isinstance(pdb_ids, str):
        pdb_ids = [pdb_ids]
    ...
    if verbose:
        print ("Determining number of entities in", len(pdb_ids), "PDB files")

    pdb_ids = map(str.upper, pdb_ids)
    pdb_ids = filter(lambda s: len(s) == 4, pdb_ids)

    # uniprot_to_pdb_df = pd.read_csv(mapping_csv_filename, index_col=0)

    # # TODO
    # pass

    pdb_to_uniprot_mapping = map_pdb_id_to_uniprot_accession(
        pdb_ids=pdb_ids,
        verbose=verbose)

    return {
        pdb_id: len(accessions)
        for pdb_id, accessions in pdb_to_uniprot_mapping.items()
    }
...
def read_from_text_file(
    filename: str,
    return_as_set: bool = True,
    ):
    """Read a text file into a list (or optionally a set)

    Parameters
    ----------
    filename : str
        The path of the file to read
    return_as_set : bool, optional
        Flag to return the elements as a set, rather than a list, by default True

    Returns
    -------
    set
        Set of string from the text file
    """
...
def cleanup_target_for_docking(
    ...
    use_foldx_to_improve_protein_structure: bool = False,
    ...
    if use_foldx_to_improve_protein_structure:
        # run foldx here
        target_filename = run_foldx(
            target_filename,
            output_pdb_filename=target_filename)
...
def prepare_single_target(
    ...
    for chain_to_process in all_chains_to_process:
        ...
        if download_pdb_structure:
            ...
            # compute SIFPs for all natural ligands (with prepared_target_filename)
            # load in protein
            ifp_type = "SIFP"
            protein = read_protein(prepared_target_filename)
            for natural_ligand_id, natural_ligand_data in target_data["natural_ligands"].items():
                if ifp_type in natural_ligand_data:
                    continue # already computed
                natural_ligand_pdb_filename = natural_ligand_data["pdb_filename"]
                natural_ligand_ifp = None
                if os.path.exists(natural_ligand_pdb_filename):
                    try:
                        natural_ligand_ifp = compute_ifp_using_oddt(
                            ligand_filename=natural_ligand_pdb_filename,
                            protein_filename=prepared_target_filename,
                            protein=protein, # preloaded 
                            ifp_type=ifp_type,
                            ifp_filename=None,
                        )
                        # convert to list 
                        natural_ligand_ifp = list(natural_ligand_ifp)
                    except Exception as e:
                        pass 
                    # delete ligand PDB file for now 
                    delete_file(natural_ligand_pdb_filename, verbose=verbose)
                natural_ligand_data[ifp_type] = natural_ligand_ifp
        ...
        if desired_chain_decompress_to_directory is not None:
            ...
            if cofactors_to_keep is not None:
                ...
                for cofactor_to_keep in cofactors_to_keep:
                    print("processing cofactor", cofactor_to_keep)
                ...
                if len(cofactor_ligand_filenames) > 0:
                    ...
                    # target_decompressed_filename = create_complex_with_pymol(
                    #     input_pdb_files=[target_decompressed_filename] + cofactor_ligand_filenames,
                    #     output_pdb_filename=target_decompressed_filename,
                    #     verbose=verbose,
                    # )
...
def rank_all_pdb_structures_for_uniprot_accessions(
    ...
    for accession, pdb_data in uniprot_to_pdb_data.items():
        ...
        #     print (pdb_id, pdb_rank_data_for_current_uniprot_accession[pdb_id]["rank"])

        # raise Exception(pdb_ids_sorted_by_rank)
