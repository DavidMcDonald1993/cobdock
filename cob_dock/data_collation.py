...
from ai_blind_docking.algorithms.cob_dock.reverse_vina import execute_reverse_docking_vina
from ai_blind_docking.algorithms.cob_dock.reverse_galaxydock import execute_reverse_docking_galaxydock
from ai_blind_docking.algorithms.cob_dock.reverse_plants import execute_reverse_docking_plants
from ai_blind_docking.algorithms.cob_dock.reverse_zdock import execute_reverse_docking_zdock
    ...
    for ligand_id, source_data_ligand in source_data.items():
        ...
        for accession, source_data_ligand_accession in source_data_ligand.items():
            ...
            for pdb_id, source_data_ligand_accession_pdb_id in source_data_ligand_accession.items():
                ...
                # print ("sorted_target_pose_ids", sorted_target_pose_ids)
                ...
                if len(sorted_target_pose_ids) == 1 and (sorted_target_pose_ids[0] == "null" or sorted_target_pose_ids[0] is None): 
                    ...
                    # print ("before", algorithm_name, is_molecular_docking_program, source_poses_as_array.shape, target_poses_as_array.shape, target_poses_as_array.dtype)
                    # print (algorithm_name, "source", ligand_id, accession, pdb_id, np.isnan(source_poses_as_array).any())
                    # print (algorithm_name, "target",  ligand_id, accession, pdb_id, np.isnan(target_poses_as_array).any())
        ...
        for accession in all_ligand_target_voxels[ligand_id]:
            ...
            for pdb_id in all_ligand_target_voxels[ligand_id][accession]:
                ...
                for voxel_id in all_ligand_target_voxels[ligand_id][accession][pdb_id]:
                    ...
                    # all_ligand_target_voxels[ligand_id][accession][pdb_id][sampled_point_id][attribute_name] = []
                    # # initialise count 
                    # all_ligand_target_voxels[ligand_id][accession][pdb_id][sampled_point_id][count_attribute_name] = 0
    ...
    for ligand_id, all_voxels_for_ligand in all_ligand_target_voxels.items():
        ...
        for accession, all_voxels_ligand_accession in all_voxels_for_ligand.items():
            ...
            for pdb_id, all_voxels_ligand_accession_pdb_id in all_voxels_ligand_accession.items():
                ...
                # print (count_attribute_name, "computing dists for", ligand_id, accession, pdb_id)
                # print ("voxel",np.isnan(voxels_as_array).any())
                # print ("pose",np.isnan(computed_pose_locations_as_array).any())
                ...
                #     print (sorted_computed_pose_ids)
                #     raise e
...
def extract_target_data_from_ligands_to_targets_data_structure(
    ...
    #     if verbose:
    #         print (data_filename, "already exists, loading it")
    #     return load_json(data_filename, verbose=verbose)
    ...
    for ligand_id, ligand_voxels in all_ligand_target_voxels.items():
        ...
        for accession, ligand_accession_voxels in ligand_voxels.items():
            ...
            for pdb_id, ligand_accession_pdb_id_voxels in ligand_accession_voxels.items():
                ...
                # if os.path.exists(all_voxel_data_for_pair_filename):
                #     continue
                ...
                # add data filename to all_pair_data_filenames
                # all_pair_data_filenames[pair_id] = all_voxel_data_for_pair_filename)
